#!/usr/bin/env python3
"""Generate docs/media/benchmark_animation.svg — animated benchmark showcase.
...
Usage:
    python docs/generation/generate_benchmark_svg.py > docs/media/benchmark_animation.svg
"""
...
def smil_visibility(active_slot, stagger_s=0):
    ...
    return (
        f'<animate attributeName="visibility" values="{values}" '
...
def generate_schedule_comment():
    ...
    lines.append("<!--")
    lines.append(
        f"  SCHEDULE — {SLOT_SECONDS}s per slot · {TOTAL_DUR}s total ({len(SLOTS)} slots)"
    )
    lines.append("  Generated by: python docs/generation/generate_benchmark_svg.py")
    lines.append("  Animation: SMIL <animate> (no CSS keyframes)")
    lines.append("")
    for i, slot in enumerate(SLOTS):
        model = slot["model"]
        labels = [f"{ADAPTERS[sid]['label']}" for sid in slot["adapters"]]
        lines.append(
            f"  Slot {i + 1:2d} [{model:>3s}]: {labels[0]:20s} {labels[1]:20s} {labels[2]:20s} {labels[3]}"
        )
    lines.append("-->")
    return "\n".join(lines)
...
def generate_left_panel():
    ...
    lines.append(
        '<text x="137" y="28" text-anchor="middle" font-size="10" fill="#8d8d8d" letter-spacing="1">CUSTOM GRANITE SWITCH MODEL</text>'
    )
    lines.append("")
    ...
    lines.append(
        '<rect x="22" y="248" width="230" height="106" rx="12" fill="#0F62FE"/>'
    )

    for si, slot in enumerate(SLOTS):
        model = slot["model"]
        minfo = MODELS[model]
        lines.append('<g visibility="hidden">')
        lines.append(f"  {smil_visibility(si)}")
        lines.append(
            f'  <text x="137" y="277" text-anchor="middle" font-size="16" font-weight="700" fill="white">{minfo["label"]}</text>'
        )
        lines.append(
            '  <text x="137" y="297" text-anchor="middle" font-size="10" fill="#a8c4ff">base model</text>'
        )
        lines.append(
            f'  <text x="137" y="315" text-anchor="middle" font-size="10" fill="#6ea6ff">{minfo["params"]}</text>'
        )
        lines.append(
            '  <text x="137" y="336" text-anchor="middle" font-size="9" fill="#4f83cc">ibm-granite \u00b7 HuggingFace</text>'
        )
        lines.append("</g>")
    lines.append("")
    ...
    for row_idx in range(4):
        ...
        lines.append(
            f"<!-- Adapter box row {row_idx} (appears {stagger}s into slot) -->"
        )
        lines.append('<g visibility="hidden">')
        ...
        lines.append(
            f'  <animate attributeName="visibility" values="{vis_values}" keyTimes="{vis_kt}" calcMode="discrete" dur="{TOTAL_DUR}s" repeatCount="indefinite"/>'
        )
        lines.append(
            f'  <rect x="22" y="{y}" width="230" height="48" rx="8" fill="{fills[0]}">'
        )
        ...
        for si, aid in enumerate(adapter_ids):
            ...
            lines.append(
                f'    <text x="137" y="{t1y}" text-anchor="middle" font-size="12" font-weight="700" fill="{lib["text"]}">{info["label"]}</text>'
            )
            lines.append(
                f'    <text x="137" y="{t2y}" text-anchor="middle" font-size="9" fill="{lib["sub"]}">{info["lib"]}</text>'
            )
            lines.append("  </g>")
        lines.append("</g>")
    lines.append("")
...
def generate_right_panel():
    ...
    lines.append("")
    lines.append(
        '<line x1="280" y1="15" x2="280" y2="365" stroke="#e0e0e0" stroke-width="1"/>'
    )
    lines.append("")
    ...
    for si, slot in enumerate(SLOTS):
        ...
        lines.append(f"  {smil_visibility(si)}")
        lines.append(
            f'  <text x="530" y="28" text-anchor="middle" font-size="10" fill="#8d8d8d" letter-spacing="1">PERFORMANCE \u00b7 {minfo["label"].upper()}</text>'
        )
        lines.append("</g>")
    lines.append("")
    ...
    for row_idx in range(4):
        ...
        lines.append(
            f'<rect x="{BAR_X}" y="{base_y}" width="{BAR_TRACK_W}" height="{BAR_H}" rx="{BAR_RX}" fill="#f4f4f4"/>'
        )
        lines.append(
            f'<rect x="{BAR_X}" y="{imp_y}" width="{BAR_TRACK_W}" height="{BAR_H}" rx="{BAR_RX}" fill="#f4f4f4"/>'
        )

        # Baseline bar — grows from 0
        lines.append(
            f'<rect x="{BAR_X}" y="{base_y}" width="0" height="{BAR_H}" rx="{BAR_RX}" fill="#c6c6c6">'
        )
        ...
        lines.append(
            f'<rect x="{BAR_X}" y="{imp_y}" width="0" height="{BAR_H}" rx="{BAR_RX}" fill="{imp_fills[0]}">'
        )
        ...
        for si in range(len(SLOTS)):
            ...
            lines.append(f"  {smil_visibility(si)}")
            lines.append(
                f'  <text x="{PCT_X}" y="{BASE_TEXT_Y[row_idx]}" text-anchor="end" font-size="10" font-weight="600" fill="#8d8d8d">{base_val}%</text>'
            )
            lines.append("</g>")
        ...
        for si in range(len(SLOTS)):
            ...
            lines.append(
                f'  <text x="{PCT_X}" y="{IMP_TEXT_Y[row_idx]}" text-anchor="end" font-size="10" font-weight="700" fill="{lib["pct"]}">{imp_val}%</text>'
            )
            lines.append("</g>")
        ...
        for si in range(len(SLOTS)):
            ...
            lines.append(f"  {smil_visibility(si)}")
            lines.append(
                f'  <text x="{BAR_X}" y="{label_y}" font-size="11" font-weight="600" fill="#161616">{info["label"]}</text>'
            )
            lines.append(
                f'  <rect x="{type_x}" y="{label_y - 11}" width="{type_w}" height="14" rx="4" fill="{lib["pill_bg"]}"/>'
            )
            lines.append(
                f'  <text x="{type_x + type_w // 2}" y="{label_y - 1}" text-anchor="middle" font-size="8" font-weight="600" fill="{lib["pill_text"]}">{type_label}</text>'
            )
            lines.append(
                f'  <rect x="{pill_x}" y="{label_y - 11}" width="{pill_w}" height="14" rx="4" fill="{lib["pill_bg"]}"/>'
            )
            lines.append(
                f'  <text x="{pill_x + pill_w // 2}" y="{label_y - 1}" text-anchor="middle" font-size="8" font-weight="600" fill="{lib["pill_text"]}">{info["lib"]}</text>'
            )
            lines.append("</g>")
    ...
    lines.append(
        '<text x="310" y="372" font-size="12" fill="#a8a8a8">prompted baseline \u2192 with aLoRA/LoRA adapter</text>'
    )
    return "\n".join(lines)
...
def _render_snippet(snippet, base_x, y, func_color):
    ...
    spans = [
        f'<tspan fill="{color_map[role]}">{text}</tspan>'
        for text, role in _tokenize(snippet)
    ]
    return (
        f'<text x="{base_x}" y="{y}" font-family="{_CODE_FONT}" font-size="{_CODE_FONT_SIZE}">'
        + "".join(spans)
        + "</text>"
    )
...
def generate_code_panel():
    ...
    lines.append(
        f'<line x1="22" y1="{CODE_PANEL_Y - 2}" x2="798" y2="{CODE_PANEL_Y - 2}" stroke="#e8e8e8" stroke-width="1"/>'
    )
    lines.append(
        f'<rect x="22" y="{CODE_PANEL_Y}" width="776" height="{CODE_PANEL_H}" rx="6" fill="#f4f4f4"/>'
    )
    lines.append(
        f'<text x="34" y="{CODE_HEADER_Y}" font-size="9" fill="#8d8d8d" letter-spacing="1">MELLEA INVOCATION</text>'
    )
...
def generate_svg():
    ...
    parts = []
    parts.append(
        f'<svg width="820" height="{SVG_HEIGHT}" viewBox="0 0 820 {SVG_HEIGHT}" xmlns="http://www.w3.org/2000/svg">'
    )
    ...
    parts.append(
        f'<rect width="820" height="{SVG_HEIGHT}" rx="10" fill="none" stroke="#e0e0e0" stroke-width="1"/>'
    )
