...
class TestValidateControlLut:
    ...
    def test_longer_than_vocab_raises(self):
        ...
        Longer-than-vocab is fatal *here* even though the builder deliberately
        produces exactly that at switch construction — see
        tests/hf/test_token_exchange.py::test_sized_past_the_last_control_id_when_vocab_lags.
        This check runs pre-save, after the resize and the rebuild have closed that
        window, so the only way to still be long is a stale buffer.
        """
