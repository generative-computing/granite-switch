...
| [hello_ollama.ipynb](notebooks/hello_ollama.ipynb) | ⚠️ Experimental — Mellea adapter functions intro over a local `ollama serve` (macOS / no GPU) | 5 min | |
...
| [rag_flow_ollama.ipynb](notebooks/rag_flow_ollama.ipynb) | ⚠️ Experimental — the full RAG flow over a local `ollama serve` (macOS / no GPU) | 5 min | |
